...
async function loadSample(){const response=await fetch("/tutorial/line-sticker/assets/pixel-demo-03.png"),blob=await response.blob(),dataUrl=await blobToDataUrl(blob);state.stickers=Array.from({length:8},(_,index)=>({name:`${String(index+1).padStart(2,"0")}.png`,blob,dataUrl}));$("#htmlCode").value=defaultHtml();renderTray();refreshPrompt();updatePreview();}
generatePalette();
